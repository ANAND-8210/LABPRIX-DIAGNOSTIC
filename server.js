const express = require("express");
const cors = require("cors");
const dotenv = require("dotenv");
const fs = require("fs/promises");
const https = require("https");
const path = require("path");
const { randomUUID } = require("crypto");
const mongoose = require("mongoose");
const Booking = require("./server/models/Booking");

dotenv.config();

const app = express();
const PORT = Number(process.env.PORT) || 10000;
const TEST_TYPES = ["Blood Test", "Sugar Test", "Full Body Checkup"];
const WHATSAPP_NUMBER = "8454822399";
const WHATSAPP_NUMBER_WITH_COUNTRY_CODE = `91${WHATSAPP_NUMBER}`;
const DATA_DIRECTORY = path.join(__dirname, "data");
const DATA_FILE = path.join(DATA_DIRECTORY, "bookings.json");
const WHATSAPP_PROVIDER = (process.env.WHATSAPP_PROVIDER || "").trim().toLowerCase();

let storageMode = "json";
let databaseConnected = false;
let jsonWriteQueue = Promise.resolve();

app.use(cors());
app.use(express.json({ limit: "1mb" }));

const normalizeBookingInput = (body = {}) => ({
  name: typeof body.name === "string" ? body.name.trim() : "",
  phone: typeof body.phone === "string" ? body.phone.trim() : "",
  email: typeof body.email === "string" ? body.email.trim().toLowerCase() : "",
  testType: typeof body.testType === "string" ? body.testType.trim() : "",
  date: typeof body.date === "string" ? body.date.trim() : "",
  time: typeof body.time === "string" ? body.time.trim() : "",
  address: typeof body.address === "string" ? body.address.trim() : ""
});

const getTodayInIndia = () => {
  const parts = new Intl.DateTimeFormat("en-US", {
    timeZone: "Asia/Kolkata",
    year: "numeric",
    month: "2-digit",
    day: "2-digit"
  }).formatToParts(new Date());

  const partMap = parts.reduce((result, part) => {
    if (part.type !== "literal") {
      result[part.type] = part.value;
    }

    return result;
  }, {});

  return `${partMap.year}-${partMap.month}-${partMap.day}`;
};

const isValidDateString = (value = "") => {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(value)) {
    return false;
  }

  const parsedDate = new Date(`${value}T00:00:00Z`);
  return !Number.isNaN(parsedDate.getTime()) && parsedDate.toISOString().slice(0, 10) === value;
};

const isValidTimeString = (value = "") => /^([01]\d|2[0-3]):([0-5]\d)$/.test(value);

const validateBookingInput = (booking) => {
  const errors = [];
  const phoneDigits = booking.phone.replace(/\D/g, "");
  const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

  if (booking.name.length < 2) {
    errors.push("Full name is required.");
  }

  if (!booking.phone) {
    errors.push("Phone number is required.");
  } else if (phoneDigits.length < 10 || phoneDigits.length > 15) {
    errors.push("Phone number must contain 10 to 15 digits.");
  }

  if (!booking.email) {
    errors.push("Email is required.");
  } else if (!emailPattern.test(booking.email)) {
    errors.push("Please provide a valid email address.");
  }

  if (!TEST_TYPES.includes(booking.testType)) {
    errors.push("Please choose a valid test type.");
  }

  if (!booking.date) {
    errors.push("Date is required.");
  } else if (!isValidDateString(booking.date)) {
    errors.push("Please provide a valid date.");
  } else if (booking.date < getTodayInIndia()) {
    errors.push("Date cannot be in the past.");
  }

  if (!booking.time) {
    errors.push("Time is required.");
  } else if (!isValidTimeString(booking.time)) {
    errors.push("Please provide a valid time.");
  }

  if (!booking.address) {
    errors.push("Address is required.");
  } else if (booking.address.length < 10) {
    errors.push("Address must be at least 10 characters long.");
  }

  return errors;
};

const buildWhatsAppMessage = (booking) =>
  [
    "New Booking:",
    `Name: ${booking.name}`,
    `Phone: ${booking.phone}`,
    `Email: ${booking.email}`,
    `Test: ${booking.testType}`,
    `Date: ${booking.date}`,
    `Time: ${booking.time}`,
    `Address: ${booking.address}`
  ].join("\n");

const buildWhatsAppFallbackUrl = (booking) =>
  `https://wa.me/${WHATSAPP_NUMBER_WITH_COUNTRY_CODE}?text=${encodeURIComponent(
    buildWhatsAppMessage(booking)
  )}`;

const ensureJsonStorage = async () => {
  await fs.mkdir(DATA_DIRECTORY, { recursive: true });

  try {
    await fs.access(DATA_FILE);
  } catch (error) {
    await fs.writeFile(DATA_FILE, "[]\n", "utf8");
  }
};

const readJsonBookings = async () => {
  await ensureJsonStorage();
  const fileContents = await fs.readFile(DATA_FILE, "utf8");
  const parsedBookings = JSON.parse(fileContents || "[]");

  return Array.isArray(parsedBookings) ? parsedBookings : [];
};

const saveBookingToJson = async (booking) => {
  const persistBooking = async () => {
    const existingBookings = await readJsonBookings();
    const savedBooking = {
      id: randomUUID(),
      ...booking,
      createdAt: new Date().toISOString()
    };

    existingBookings.push(savedBooking);
    await fs.writeFile(DATA_FILE, `${JSON.stringify(existingBookings, null, 2)}\n`, "utf8");

    return savedBooking;
  };

  const queuedWrite = jsonWriteQueue.then(persistBooking, persistBooking);
  jsonWriteQueue = queuedWrite.catch(() => {});

  return queuedWrite;
};

const connectToMongoIfAvailable = async () => {
  const mongoUri = (process.env.MONGODB_URI || "").trim();

  if (!mongoUri) {
    storageMode = "json";
    databaseConnected = false;
    return;
  }

  try {
    await mongoose.connect(mongoUri, {
      serverSelectionTimeoutMS: 5000
    });

    storageMode = "mongodb";
    databaseConnected = true;
    console.log("MongoDB connected successfully.");
  } catch (error) {
    storageMode = "json";
    databaseConnected = false;
    console.warn("MongoDB connection failed. Falling back to JSON storage.");
    console.warn(error.message);
  }
};

const saveBooking = async (booking) => {
  if (storageMode === "mongodb" && mongoose.connection.readyState === 1) {
    return Booking.create(booking);
  }

  return saveBookingToJson(booking);
};

const sendHttpsRequest = (url, { method = "POST", headers = {}, body = "" } = {}) =>
  new Promise((resolve, reject) => {
    const request = https.request(url, { method, headers }, (response) => {
      let responseBody = "";

      response.on("data", (chunk) => {
        responseBody += chunk;
      });

      response.on("end", () => {
        let responseJson = null;

        if (responseBody) {
          try {
            responseJson = JSON.parse(responseBody);
          } catch (error) {
            responseJson = null;
          }
        }

        resolve({
          statusCode: response.statusCode || 500,
          body: responseBody,
          json: responseJson
        });
      });
    });

    request.on("error", reject);

    if (body) {
      request.write(body);
    }

    request.end();
  });

const sendViaTwilio = async (message) => {
  const accountSid = (process.env.TWILIO_ACCOUNT_SID || "").trim();
  const authToken = (process.env.TWILIO_AUTH_TOKEN || "").trim();
  const fromNumber = (process.env.TWILIO_WHATSAPP_FROM || "").trim();
  const toNumber = (
    process.env.TWILIO_WHATSAPP_TO || `whatsapp:+${WHATSAPP_NUMBER_WITH_COUNTRY_CODE}`
  ).trim();

  if (!accountSid || !authToken || !fromNumber) {
    return {
      status: "skipped",
      provider: "twilio",
      message: "Twilio WhatsApp credentials are not configured."
    };
  }

  const body = new URLSearchParams({
    To: toNumber,
    From: fromNumber,
    Body: message
  }).toString();

  const response = await sendHttpsRequest(
    `https://api.twilio.com/2010-04-01/Accounts/${accountSid}/Messages.json`,
    {
      method: "POST",
      headers: {
        Authorization: `Basic ${Buffer.from(`${accountSid}:${authToken}`).toString("base64")}`,
        "Content-Type": "application/x-www-form-urlencoded",
        "Content-Length": Buffer.byteLength(body)
      },
      body
    }
  );

  if (response.statusCode >= 200 && response.statusCode < 300) {
    return {
      status: "sent",
      provider: "twilio",
      message: "WhatsApp notification sent successfully."
    };
  }

  throw new Error(
    response.json?.message || "Twilio WhatsApp API returned an unexpected error."
  );
};

const sendViaWhatsAppCloud = async (message) => {
  const phoneNumberId = (process.env.WHATSAPP_PHONE_NUMBER_ID || "").trim();
  const accessToken = (process.env.WHATSAPP_CLOUD_TOKEN || "").trim();
  const apiVersion = (process.env.WHATSAPP_CLOUD_VERSION || "v23.0").trim();

  if (!phoneNumberId || !accessToken) {
    return {
      status: "skipped",
      provider: "whatsapp-cloud",
      message: "WhatsApp Cloud API credentials are not configured."
    };
  }

  const payload = JSON.stringify({
    messaging_product: "whatsapp",
    to: WHATSAPP_NUMBER_WITH_COUNTRY_CODE,
    type: "text",
    text: {
      preview_url: false,
      body: message
    }
  });

  const response = await sendHttpsRequest(
    `https://graph.facebook.com/${apiVersion}/${phoneNumberId}/messages`,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${accessToken}`,
        "Content-Type": "application/json",
        "Content-Length": Buffer.byteLength(payload)
      },
      body: payload
    }
  );

  if (response.statusCode >= 200 && response.statusCode < 300) {
    return {
      status: "sent",
      provider: "whatsapp-cloud",
      message: "WhatsApp notification sent successfully."
    };
  }

  throw new Error(
    response.json?.error?.message || "WhatsApp Cloud API returned an unexpected error."
  );
};

const sendWhatsAppNotification = async (message) => {
  try {
    if (WHATSAPP_PROVIDER === "twilio") {
      return await sendViaTwilio(message);
    }

    if (WHATSAPP_PROVIDER === "cloud" || WHATSAPP_PROVIDER === "meta") {
      return await sendViaWhatsAppCloud(message);
    }

    return {
      status: "skipped",
      provider: "none",
      message: "Set WHATSAPP_PROVIDER to twilio or cloud to enable WhatsApp delivery."
    };
  } catch (error) {
    return {
      status: "failed",
      provider: WHATSAPP_PROVIDER || "unknown",
      message: error.message || "Unable to send WhatsApp notification."
    };
  }
};

app.get("/", (req, res) => {
  res.status(200).json({
    success: true,
    message: "Blood collection booking API is running."
  });
});

app.get(["/health", "/api/health"], (req, res) => {
  res.status(200).json({
    success: true,
    storageMode,
    databaseConnected,
    whatsappProvider: WHATSAPP_PROVIDER || "not-configured"
  });
});

app.post("/book", async (req, res) => {
  const booking = normalizeBookingInput(req.body);
  const validationErrors = validateBookingInput(booking);

  if (validationErrors.length > 0) {
    return res.status(400).json({
      success: false,
      message: validationErrors[0],
      errors: validationErrors
    });
  }

  try {
    const savedBooking = await saveBooking(booking);
    const notification = await sendWhatsAppNotification(buildWhatsAppMessage(booking));

    return res.status(201).json({
      success: true,
      message: "Booking saved successfully.",
      booking: savedBooking,
      notification,
      fallbackUrl: buildWhatsAppFallbackUrl(booking)
    });
  } catch (error) {
    console.error("Booking request failed:", error);

    return res.status(500).json({
      success: false,
      message: "Unable to process booking right now. Please try again in a moment.",
      fallbackUrl: buildWhatsAppFallbackUrl(booking)
    });
  }
});

app.use((req, res) => {
  res.status(404).json({
    success: false,
    message: "Route not found."
  });
});

const startServer = async () => {
  await ensureJsonStorage();
  await connectToMongoIfAvailable();

  app.listen(PORT, () => {
    console.log(`Blood collection booking server is running on port ${PORT}.`);
    console.log(`Storage mode: ${storageMode}`);
  });
};

startServer().catch((error) => {
  console.error("Server failed to start:", error);
  process.exit(1);
});
